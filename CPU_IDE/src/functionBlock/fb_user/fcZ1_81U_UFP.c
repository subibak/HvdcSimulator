            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0705_CODE;
    fbInfoPtr->inputNo		= FC0705_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0705_VAR_NUM;
    fbInfoPtr->outputNo		= FC0705_OUTPUT_NUM;
...
uint32	stdZ1_81U_UFPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			/* Fault Detection */
			fbInfo.o1_alarm 	= ftUnderDetect(fbInfo.freq, fbInfo.threshold1, fbInfo.setT1, &fbInfo.t1, cycleTimeSec);	
			fbInfo.o2_conBlk 	= ftUnderDetect(fbInfo.freq, fbInfo.threshold2, fbInfo.setT2, &fbInfo.t2, cycleTimeSec);	
    	}
    }
