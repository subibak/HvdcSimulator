...
**
**********************************************************************/
uint32	stdZ2_26W_TTOLPInitFunc
				(
			uint32			*inputTypePtr,
            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0711_CODE;
    fbInfoPtr->inputNo		= FC0711_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0711_VAR_NUM;
    fbInfoPtr->outputNo		= FC0711_OUTPUT_NUM;
...
uint32	stdZ2_26W_TTOLPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    if (reset == 1)
    {
		...
		fbInfo.o2_trip.a	= NORMAL;
		fbInfo.o2_trip.b	= NORMAL;
		fbInfo.o2_trip.c	= NORMAL;
	}
    else
    {
    	if(enable == 1)
    	{
			...
			/* Fault Detection */
			fbInfo.o1_alarm.a = ftOverDetect(fbInfo.ittr.a, fbInfo.threshold1, fbInfo.setT1, &fbInfo.t1.a, cycleTimeSec);
			fbInfo.o1_alarm.b = ftOverDetect(fbInfo.ittr.b, fbInfo.threshold1, fbInfo.setT1, &fbInfo.t1.b, cycleTimeSec);
			fbInfo.o1_alarm.c = ftOverDetect(fbInfo.ittr.c, fbInfo.threshold1, fbInfo.setT1, &fbInfo.t1.c, cycleTimeSec);
			fbInfo.o2_trip.a  = ftOverDetect(fbInfo.ittr.a, fbInfo.threshold2, fbInfo.setT2, &fbInfo.t2.a, cycleTimeSec);
			fbInfo.o2_trip.b  = ftOverDetect(fbInfo.ittr.b, fbInfo.threshold2, fbInfo.setT2, &fbInfo.t2.b, cycleTimeSec);
			fbInfo.o2_trip.c  = ftOverDetect(fbInfo.ittr.c, fbInfo.threshold2, fbInfo.setT2, &fbInfo.t2.c, cycleTimeSec);
    	}
    }
