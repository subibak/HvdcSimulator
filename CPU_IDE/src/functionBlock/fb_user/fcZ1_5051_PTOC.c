...
**
**********************************************************************/
uint32	stdZ1_5051_PTOCInitFunc
				(
			uint32			*inputTypePtr,
            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0706_CODE;
    fbInfoPtr->inputNo		= FC0706_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0706_VAR_NUM;
    fbInfoPtr->outputNo		= FC0706_OUTPUT_NUM;
...
uint32	stdZ1_5051_PTOCRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			inv_ipcc.a = fbInfo.ipcc.a;
			inv_ipcc.b = fbInfo.ipcc.b;
			inv_ipcc.c = fbInfo.ipcc.c;	
					
			/* Fault Detection */
			fbInfo.o1_emst.a = ftOverDetect(fbInfo.ipcc.a, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.a, cycleTimeSec);
			fbInfo.o1_emst.b = ftOverDetect(fbInfo.ipcc.b, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.b, cycleTimeSec);
			fbInfo.o1_emst.c = ftOverDetect(fbInfo.ipcc.c, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.c, cycleTimeSec);
			fbInfo.o2_emst.a = ftOverDetect(inv_ipcc.a, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.a, cycleTimeSec);
			fbInfo.o2_emst.b = ftOverDetect(inv_ipcc.b, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.b, cycleTimeSec);
			fbInfo.o2_emst.c = ftOverDetect(inv_ipcc.c, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.c, cycleTimeSec);    	}
    }
