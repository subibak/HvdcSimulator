...
**
**********************************************************************/
uint32	stdZ2_DcSat_TDSPInitFunc
				(
			uint32			*inputTypePtr,
            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0712_CODE;
    fbInfoPtr->inputNo		= FC0712_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0712_VAR_NUM;
    fbInfoPtr->outputNo		= FC0712_OUTPUT_NUM;
...
uint32	stdZ2_DcSat_TDSPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			/* Fault Detection */			
			fbInfo.o1_emst = ftOverDetect(fbInfo.idc_tr, fbInfo.threshold, fbInfo.setT, &fbInfo.t1, cycleTimeSec);
    	}
    }
