...
**
**********************************************************************/
uint32	stdZ3_51Tharm_AHCPInitFunc
				(
			uint32			*inputTypePtr,
            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0721_CODE;
    fbInfoPtr->inputNo		= FC0721_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0721_VAR_NUM;
    fbInfoPtr->outputNo		= FC0721_OUTPUT_NUM;
...
uint32	stdZ3_51Tharm_AHCPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
