            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0703_CODE;
    fbInfoPtr->inputNo		= FC0703_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0703_VAR_NUM;
    fbInfoPtr->outputNo		= FC0703_OUTPUT_NUM;
...
uint32	stdZ1_64_OVGRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			/* Fault Detection */
			fbInfo.o1_alarm = ftOverDetect(fbInfo.vrms0, fbInfo.threshold1, fbInfo.setT1, &fbInfo.t1, cycleTimeSec);	
    	}
    }
