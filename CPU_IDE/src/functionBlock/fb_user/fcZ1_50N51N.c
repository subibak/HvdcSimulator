            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0707_CODE;
    fbInfoPtr->inputNo		= FC0707_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0707_VAR_NUM;
    fbInfoPtr->outputNo		= FC0707_OUTPUT_NUM;
...
uint32	stdZ1_50N51NRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			/* Fault Detection */
			fbInfo.o1_emst = ftOverDetect(fbInfo.itr_n, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1, cycleTimeSec);
			fbInfo.o2_emst = ftOverDetect(fbInfo.itr_n, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2, cycleTimeSec);			
    	}
    }
