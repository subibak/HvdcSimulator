            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0732_CODE;
    fbInfoPtr->inputNo		= FC0732_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0732_VAR_NUM;
    fbInfoPtr->outputNo		= FC0732_OUTPUT_NUM;
...
uint32	stdZ4_51AharmRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
