            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0719_CODE;
    fbInfoPtr->inputNo		= FC0719_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0719_VAR_NUM;
    fbInfoPtr->outputNo		= FC0719_OUTPUT_NUM;
...
uint32	stdZ3_51G_GOCPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			fbInfo.o2_emst	= ftOverDetect(ABS_igr, fbInfo.threshold2, fbInfo.setT2, &fbInfo.t2, cycleTimeSec);
    	}
    }
