...
**
**********************************************************************/
uint32	stdZ3_50S51S_OCPInitFunc
				(
			uint32			*inputTypePtr,
            	...
            	)
{
	...
	**	FB 정보 정의
	**************************************************/
	fbInfoPtr->fbId			= FC0717_CODE;
    fbInfoPtr->inputNo		= FC0717_SPEC_NUM;
    fbInfoPtr->intlVarNo	= FC0717_VAR_NUM;
    fbInfoPtr->outputNo		= FC0717_OUTPUT_NUM;
...
uint32	stdZ3_50S51S_OCPRunFunc(uint32 taskId, uint32 fbMemAddr)
{
	uint32	status = NO_ERROR;
	float	cycleTimeSec;
	float	reset;
    ...
    else
    {
    	if(enable == 1)
    	{
			...
			inv_itr_2nd.a = fbInfo.itr_2nd.a;
			inv_itr_2nd.b = fbInfo.itr_2nd.b;
			inv_itr_2nd.c = fbInfo.itr_2nd.c;	

			/* Fault Detection */
			fbInfo.o1_emst.a = ftOverDetect(fbInfo.itr_2nd.a, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.a, cycleTimeSec);
			fbInfo.o1_emst.b = ftOverDetect(fbInfo.itr_2nd.b, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.b, cycleTimeSec);
			fbInfo.o1_emst.c = ftOverDetect(fbInfo.itr_2nd.c, fbInfo.threshold1, fbInfo.setT, &fbInfo.t1.c, cycleTimeSec);
			fbInfo.o2_emst.a = ftOverDetect(inv_itr_2nd.a, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.a, cycleTimeSec);
			fbInfo.o2_emst.b = ftOverDetect(inv_itr_2nd.b, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.b, cycleTimeSec);
			fbInfo.o2_emst.c = ftOverDetect(inv_itr_2nd.c, fbInfo.threshold2, fbInfo.setT, &fbInfo.t2.c, cycleTimeSec);
    	}
    }
