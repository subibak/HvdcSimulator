...
#define CRC16_COEFF1		8
#define CRC16_COEFF2		0xFF
#define CRC16_COEFF3		0xFF00
...
{
	register uint16_t wCrc = 0;
	uint32_t ulLoop;

	for(ulLoop = 0; ulLoop < ulLen; ulLoop++)
	{
	
//		wCrc = ((wCrc << CRC16_COEFF1) & 0xFF00) ^ (crc16tab[(((wCrc >> CRC16_COEFF1) & 0xFF) ^ (*bBuf & 0xFF)) & 0xFF] & 0xFFFF);

		wCrc = ((wCrc << CRC16_COEFF1)& CRC16_COEFF3 ) ^ (awCrc16Table[(((wCrc >> CRC16_COEFF1) & CRC16_COEFF2) ^ (bBuf[ulLoop] & CRC16_COEFF2)) & CRC16_COEFF2] & CRC16_INIT_VALUE);
...
//	uint16_t *pwCrcData;
	...
	return Crc16Ccitt(bBuf, ulLen);
//	bBuf += ulLen;
//	pwCrcData = (uint16_t *) bBuf;
//	pwCrcData = bBuf[ulLen];
//	*pwCrcData = wCrc;
...
int32_t CheckCrc16(uint32_t ulCrc, uint8_t *bBuf, int32_t sz)
{
	uint32_t	ulLoop;
	uint8_t 	cks=0;
	uint8_t		ccrc=0, tcrc=0;

	if (ulCrc)
	{
		ccrc  = Crc16Ccitt(bBuf, sz);
		tcrc = ((bBuf[sz] << 8) & 0xFF00) + bBuf[sz+1];
		if (ccrc == tcrc)		return true;	// success
	}
	else
	{
		for (ulLoop=0; ulLoop < sz; ++ulLoop)	cks += bBuf[ulLoop];
		if (cks == bBuf[sz])		return true;	// success
	}
...
#if 0
uint32_t CheckCrc16(uint16_t *bBuf, uint32_t ulLen)
{
	uint16_t wCrc;
	uint32_t ulResult = CRC_OK;

	wCrc = Crc16Ccitt(bBuf, ulLen);

//	bBuf += ulLen;
//	pwCrcData = (uint16_t *) bBuf;

//	if (wCrc == *pwCrcData) 	return CRC_OK;

//	PRINTF("CRC ERROR : MK(0x%04X), RCV(0x%04X)\n", crc, *crc_data);
...
uint32_t make_crc32(uint8_t *buf, uint32_t len)
{
	uint16_t	crc;
	crc = Crc16Ccitt(buf, len);
	return ((crc << 16) + crc);
}
...
#define CRC32_COEFF1		8
#define CRC32_COEFF2		24
#define CRC32_COEFF3		0xFF


// All zero Data 에 대한 CRC 계산 시 에러 발생함.
uint32_t MakeCrc32(const uint32_t *pulBuf, int len)
{

	uint32_t ulCrc = CRC32_INIT_VALUE;
	uint32_t ulLoop;

	for(ulLoop = 0; ulLoop < len; ulLoop++)
	{
		ulCrc += pulBuf[ulLoop];
//		ulCrc = (ulCrc << CRC32_COEFF1) ^ aulCrc32Table[((ulCrc >> CRC32_COEFF2) ^ pulBuf[ulLoop]) & CRC32_COEFF3];
	}
